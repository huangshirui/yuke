...
对外 UI 使用通用预约语言，避免教育行业特定角色词。内部 Contract 与代码继续使用稳定 Domain 名称；小程序和 Web Admin 分别使用面向其任务的产品术语，不把内部建模词直接暴露给用户。

| Domain | 小程序用户 UI | Web Admin UI | English / internal |
|---|---|---|---|
| Space | 服务方 | 服务方 | Space |
| Resource | 预约项目 / 预约什么 | 预约项目 | Resource |
| Participant | 常用预约人 / 为谁预约 | 预约人 | Participant |
| Slot | 可预约时间 | 时段 | Slot |
| Slot Type | — | 时段类型 | Slot Type |
| Booking | 预约 | 预约 | Booking |
| Customer (Mini Program User) | 用户 | 客户 | Customer |
| Web Admin User | — | 用户 | User |
| Space Admin | — | 服务方用户 | Space User |
...
Web Admin 对外术语固定为：后台登录账号一律称“用户”，微信小程序侧账号一律称“客户”；Space / Resource / Participant / Slot 分别显示为“服务方 / 预约项目 / 预约人 / 时段”。“时段类型”保留为管理端专业术语。页面不得暴露认证供应商名、内部类型名（如 `AdminUser`）或 `adm*` / `mem*` / `spc*` 等内部技术 ID。

后台“用户”使用**用户名称**作为主要识别信息，邮箱只作为登录账号、账号匹配及必要时的次级信息；旧数据尚未设置名称时可以回退显示邮箱。客户的“来源用户”属于加入时固化的历史来源，身份解析不得依赖当前 Space 用户列表：Super Admin 是平台级用户，本来就可以不出现在 `space_admins` 中；普通用户后续权限变化也不应影响历史来源展示。任何情况下都不应因此显示“已移除用户”或内部 ID。
...
Slot 是预约工作台的核心时间资源，但 UI 不应把所有业务状态塞进一个 SlotStatus。Web Admin 对外统一称“时段”；底层 `frozen` 在 UI 中表达为“已暂停 / 暂停时段 / 恢复时段”。日历展示优先级：

1. 没有有效 Booking 时，卡片固定显示**时段类型**；可预约性通过 Hover / Focus 和详情表达；
2. `booked` Booking 时，卡片只固定显示**预约人名称**，不重复显示客户名称，也不显示“已预约”文字；
3. `completed` Booking 仍以预约人名称为主，并紧凑显示 **待对账 / 已对账**，避免丢失运营待办；
4. Frozen 是附加运营限制，可叠加在空闲、已预约或已完成状态上，不覆盖 Booking / Reconciliation 的服务事实；
5. cancelled Slot 默认不出现在主日历，可在历史/筛选场景查看。

日历状态颜色固定为四组主底色 + 一个叠加态：

- 空闲可预约：白色，主色左边线；
- 已预约：浅墨绿色；
- 已完成 · 待对账：浅黄色；
- 已完成 · 已对账：浅绿色；
- 已暂停（Frozen）：灰色虚线边框；空闲时使用浅灰底，可与其它主状态叠加。
- 没有 Booking 但暂不可预约：不增加第六种状态色，沿用空闲样式并降低文字视觉权重，Hover / Focus 明确显示“暂不可预约”。
...
- 空闲时段卡片固定显示时段类型；Hover / Focus 显示精确开始结束时间以及“可预约 / 已暂停 / 暂不可预约”。
- 已预约时段卡片固定显示预约人名称；普通 booked 状态不再额外显示“已预约”。Hover / Focus 至少显示精确时间、客户名称、预约人名称；completed 时可追加待对账 / 已对账状态。
- Desktop 周历必须以最短 30 分钟 Slot 仍清晰可读为基线：卡片正文使用单行主信息 + 必要的紧凑运营状态，不纵向堆叠“时间 / 客户 / 预约人 / 状态”。
- 名称过长时卡片单行省略，完整信息通过 Hover / Focus 和 Detail Drawer 获取。
- 颜色用于快速区分空闲、已预约、已完成待对账、已完成已对账；待对账 / 已对账同时保留文字，不能只靠颜色。
- 点击没有 Booking 的 Slot 始终在当前日历上下文打开 Slot Detail，包括暂不可预约或已暂停状态；点击 booked / completed Slot 打开 Booking Detail。
- Desktop 默认使用右侧 Detail Drawer；Mobile 使用底部 Sheet。Mobile 没有 Hover，因此通过 Agenda 行 + Detail Sheet 保留等价信息。关闭详情后必须保留当前服务方、预约项目、周次 / 日期与滚动位置。
...
- 导航分为运营、预约配置、服务方设置、数据；运营区收敛为“概览 / 预约 / 客户管理”，“预约”内部提供“日历 / 列表”两种视图；服务方设置中的后台账号统一称“用户”，使用“用户管理”；“对账”保留入口并标记即将开放。
